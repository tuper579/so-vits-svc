...
def run(rank, n_gpus, hps, cmd_args):
    ...
    scheduler_g = torch.optim.lr_scheduler.ExponentialLR(optim_g.optimizer, gamma=hps.train.lr_decay, last_epoch=epoch_str - 2)
    scheduler_d = torch.optim.lr_scheduler.ExponentialLR(optim_d.optimizer, gamma=hps.train.lr_decay, last_epoch=epoch_str - 2)
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, loaders, logger, writers):
    ...
    for batch_idx, items in enumerate(train_loader):
        ...
        spec, y = spec.cuda(rank, non_blocking=True),
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                logger.info(f"Losses: {[x.item() for x in losses]},"
                    f" step: {global_step}, lr: {lr}")
                ...
                image_dict = {
                    "slice/mel_org": sovits_utils.plot_spectrogram_to_numpy(
                        y_mel[0].data.cpu().numpy()),
                    "slice/mel_gen": sovits_utils.plot_spectrogram_to_numpy(
                        y_hat_mel[0].data.cpu().numpy()),
                    "all/mel": sovits_utils.plot_spectrogram_to_numpy(
                        mel[0].data.cpu().numpy()),
                    "all/lf0": sovits_utils.plot_data_to_numpy(
                        lf0[0, 0, :].cpu().numpy(),
                                                          pred_lf0[0, 0, :].detach().cpu().numpy()),
                    "all/norm_lf0": sovits_utils.plot_data_to_numpy(
                        lf0[0, 0, :].cpu().numpy(),
